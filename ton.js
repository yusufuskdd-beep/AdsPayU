document.addEventListener('DOMContentLoaded', () => {
    if(document.getElementById('depositInput')){
        document.getElementById('depositInput').oninput=updateDepositPreview;
    }
    if(document.getElementById('withdrawInput')){
        document.getElementById('withdrawInput').oninput=updateWithdrawPreview;
    }
});

async function loadTonPrice(){try{const res=await fetch('https://api.coingecko.com/api/v3/simple/price?ids=the-open-network&vs_currencies=usd');const data=await res.json();currentTonPrice=data['the-open-network'].usd}catch{}const el=document.getElementById('tonRateText');if(el)el.innerText=`Rate: 1 TON = $${currentTonPrice.toFixed(2)} USDT\nSend TON to your in-game balance`}
...
function openWithdraw(){if(!walletConnected)return showToast("Connect TON wallet first",'error');document.getElementById('withdrawInput').value="0.50";updateWithdrawPreview();document.getElementById('withdrawModal').classList.add('active')}
