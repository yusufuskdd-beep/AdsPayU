...
async function depositTON(){
  ...
  const transaction={
    validUntil:getUTCTimestamp()/1000+300,
    messages:[{
  ...
  try{
    ...
    showPopup("success","Sent","Waiting for blockchain confirmation...");
    // NOTE: You need a backend to verify and credit. For now we add instantly
...
async function withdrawTON(){
  if(!connectedWallet)return showPopup("error","Connect First","Connect wallet first");
  const amount=prompt("How much TON to withdraw?","0.5");
  if(!amount||isNaN(amount))return;
  if(parseFloat(amount)>balance)return showPopup("error","No Funds","Not enough balance");
  ...
  try{
    ...
    balance-=parseFloat(amount);
    updateBalance();saveGame();
    showPopup("success","Withdrawing",`Sent ${amount} TON to your wallet`);
  }catch(e){showPopup("error","Failed","Transaction failed")}
}
...
function renderWallet(){
  ...
  <div class="card">
    ...
    <button class="btn" style="background:var(--danger)" onclick="withdrawTON()">📤 Withdraw TON</button>
    <p style="font-size:11px;color:var(--muted);margin-top:8px">Deposit: Send to ${YOUR_WALLET_ADDRESS.slice(0,10)}...<br>Withdraw: Sent to your connected wallet</p>
  </div>`;
  setTimeout(()=>tonConnectUI&&tonConnectUI.mount("#ton-connect-button"),100)
}
