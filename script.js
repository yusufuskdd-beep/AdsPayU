...
const MAX_ADS_PER_DAY=50,AD_REWARD=.002;
...
}catch(t){console.error("TonConnect init error",t)}
...
document.addEventListener("DOMContentLoaded",()=>{
  try{
    ...
    showPopup("error","Init Failed", err.message)
  }
});

function checkDailyReset(){const t=getUTCDateString(Date.now());t!==lastAdResetDate&&(adsWatchedToday=0,lastAdResetDate=t)}

function loadGame(){const t=localStorage.getItem(SAVE_KEY);if(t){const e=JSON.parse(t);balance=e.balance||10,taskProgress=e.taskProgress||{},lastDailyClaim=e.lastDailyClaim||0,dailyStreak=e.dailyStreak||0,adsWatchedToday=e.adsWatchedToday||0,lastAdResetDate=e.lastAdResetDate||"",checkDailyReset(),lastDailyClaim>0&&getUTCDateString(lastDailyClaim)!==getUTCDateString(Date.now())&&getUTCDateString(lastDailyClaim)!==getUTCDateString(Date.now()-864e5)&&(dailyStreak=0)}const e=localStorage.getItem(TASK_KEY);e&&(completedTasks=JSON.parse(e)),saveGame()}
...
function renderHome(){
const i=getDailyReward(),n=getUTCDateString(lastDailyClaim)!==getUTCDateString(Date.now());
...
// FIX 2: Safe wallet render
function renderWallet(){
  const t=tonConnectUI&&tonConnectUI.connected;
  const e=t?tonConnectUI.account.address.slice(0,6)+"..."+tonConnectUI.account.address.slice(-4):"Not Connected";
  document.getElementById("content").innerHTML=`<h2>Wallet</h2><div class="card"><h3>Wallet Status</h3><p style="color:var(--muted); font-size:12px">Connected: <b>${e}</b></p><div id="ton-connect-button" style="margin-bottom:8px"></div>${!t&&tonConnectUI?'<button class="btn" onclick="connectWallet()">Connect Wallet</button>':""}${t?'<button class="btn" style="background:var(--danger)" onclick="disconnectWallet()">Disconnect</button>':""}</div>`;
  if(tonConnectUI) setTimeout(()=>{tonConnectUI.mount("#ton-connect-button")},100)
}
