...
let tonConnectUI=null;try{tonConnectUI=new TON_CONNECT_UI.TonConnectUI({manifestUrl:"https://adspayu.vercel.app/tonconnect-manifest.json"})}catch{}
...
function renderHome(){const c=document.getElementById("content");const t=getTotalRate(),e=getTotalFarmed(),mad=getTotalMadRate();c.innerHTML=`<div class="card"><h2>Welcome ${user.first_name}</h2><p>ROI 5%-18% + MAD</p></div><div class="card"><h3>⛏️ TON Mining</h3><p>Rate: <b>${(t*86400).toFixed(4)}/day</b></p><p>Farmed: <b id="farmedTotal">${e.toFixed(6)}</b></p><div class="progress"><div class="progress-bar" style="width:100%"></div></div><button class="btn" id="claimBtn" ${!gigaReady?"disabled":""} onclick="claimMiner()">${gigaReady?getClaimCooldownText():'LOADING ADS...'}</button></div><div class="card"><h3>💎 MAD Mining</h3><p>Rate: <b>${mad}/hour</b></p><p>Balance: <b>${madBalance.toFixed(0)}</b></p><button class="btn" id="claimMadBtn" ${!gigaReady?"disabled":""} onclick="claimMad()">${gigaReady?getMadCooldownText():'LOADING ADS...'}</button></div><div class="card"><h3>Your Miners</h3>${minerInstances.length?minerInstances.map(t=>`<div class="miner-unit">
  ...
  <div class="miner-info">
    <h4>${t.name} #${t.instanceId}</h4>
    <p>${(t.rate*86400).toFixed(4)}/d • ${t.madRate} MAD/h • +${(t.bonus*100).toFixed(0)}%</p>
    <p>Farmed: <span id="farmed-${t.instanceId}">${t.farmed.toFixed(6)}</span></p>
  </div>
</div>`).join(""):'<p style="color:var(--muted)">No miners yet. Buy one from Shop!</p>'}<button class="btn" onclick="buyMiner(4)">Buy GPU Rig 10 TON</button></div>`}

// 3. SHOP WITH YOUR IMAGES
function renderShop(){const c=document.getElementById("content");if(!c)return;c.innerHTML=`<h2>Shop</h2>${minerTemplates.map(t=>{const owned=minerInstances.filter(e=>e.templateId===t.id).length,roi=(t.cost*(1+t.bonus)).toFixed(4),disabled=owned>=3?"disabled":"";return`<div class="card miner">
  <img src="${t.img}" class="miner-img" onerror="this.src='micro.png'"/>
  <div class="miner-info">
    <h3>${t.name}</h3>
    <p>${(t.rate*86400).toFixed(4)} TON/day • ${t.madRate} MAD/h</p>
    <p>30d ROI: <b>${roi} TON</b> +${(t.bonus*100).toFixed(0)}%</p>
    <p><b>${t.cost} TON</b> • Owned: ${owned}/3</p>
  </div>
  <button class="btn" style="width:90px" ${disabled} onclick="buyMiner(${t.id})">${disabled?"MAX":"Buy"}</button>
</div>`}).join("")}`}
...
async function connectWallet(){await tonConnectUI.connectWallet()}
function renderWallet(){document.getElementById("content").innerHTML=`<h2>Wallet</h2><div class="card"><p>TON: ${balance.toFixed(4)}</p><p>MAD: ${madBalance.toFixed(0)}</p><div id="ton-connect-button"></div></div>`;setTimeout(()=>tonConnectUI&&tonConnectUI.mount("#ton-connect-button"),100)}
function renderReferral(){document.getElementById("content").innerHTML=`<h2>Referral</h2><div class="card"><p>Link: https://t.me/AdsPayU_bot?start=${user.id}</p></div>`}
function renderProfile(){document.getElementById("content").innerHTML=`<h2>Profile</h2><div class="card"><p>Name: ${user.first_name}</p><p>TON: ${balance.toFixed(4)}</p></div>`}
