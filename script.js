...
function renderHome(){
  ...
  const t=getTotalRate(),e=getTotalFarmed(),mad=getTotalMadRate();
  const adsReady=typeof window.showGiga==="function";
...
function renderWallet(){const c=document.getElementById("content");const isConnected=!!connectedWallet;const walletAddr=isConnected?`${connectedWallet.account.address.slice(0,4)}...${connectedWallet.account.address.slice(-4)}`:"Connect Wallet";c.innerHTML=`<h2>Wallet</h2><div class="card"><h3>💰 Game Balance</h3><p style="font-size:20px">${balance.toFixed(4)} TON</p><p style="color:var(--gold)">${madBalance.toFixed(0)} MAD</p></div><div class="card"><button class="btn" style="width:100%" onclick="${isConnected?'disconnectWallet()':'connectWallet()'}">${walletAddr}</button></div><div class="card"><h3>📥 Deposit TON</h3><p style="word-break:break-all;font-size:11px;background:#0f131a;padding:8px;border-radius:8px">${YOUR_WALLET_ADDRESS}</p><button class="btn" onclick="depositTON()" ${!isConnected?"disabled":""}>📥 Deposit TON</button></div>`;}
