...
function resetTasks() { if(confirm("Reset all tasks?")) { completedTasks = []; taskProgress = {}; saveGame(); showPopup('success', 'Reset Done', 'All tasks reset'); renderProfile(); } }
...
function renderTasks() {
  ...
  const tasks = tasksData[activeTaskTab]; let tasksHtml = tasks.length === 0? `<div class="card"><p style="text-align:center; color:var(--muted)">No tasks here yet</p></div>` : tasks.map(t => {
  ...
  }).join(''); document.getElementById('content').innerHTML = `<h2>Tasks</h2><div class="subtabs">${tabsHtml}</div>${tasksHtml}`;
}
