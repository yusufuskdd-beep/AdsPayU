...
const minerTemplates = [
  { id: 1, name: "Micro Miner", cost: 1, bonus: 0.10, rate: 0.00000424, img: "micro.png" },
...
function showRewardedAd(giveReward = true) {
  return new Promise((resolve, reject) => {
    if(isAdPlaying) return reject('busy');
    if(typeof window.showGiga!== 'function') { // SAFETY CHECK
      console.log("GigaPub not loaded yet");
...
// SAFE AUTO AD - NO REWARD, NO COUNT
async function showAutoAd() {
  if(isAdPlaying) return;
  if(typeof window.showGiga!== 'function') return; // Don't crash if not loaded
  try {
    await showRewardedAd(false);
  } catch(e) {
    console.log("Auto ad skipped:", e);
  }
}
...
function renderTasks() {
  ...
  if(activeTaskTab === 'ads') {
    ...
    tasksHtml = tasks.length === 0? `<div class="card"><p style="text-align:center; color:var(--muted)">No tasks here yet</p></div>` : tasks.map(t => {
      const claimed = completedTasks.includes(t.id); const canClaim = isTaskComplete(t); const verifyBtn =!taskProgress[t.id] &&!claimed? `<button class="btn" style="width:80px; background:#fbbf24; color:#000; margin-right:8px" onclick="markTaskProgress('${t.id}')">Verify</button>` : '';
      ...
      let claimBtnText = 'Claim'; let claimBtnDisabled =!canClaim; let claimBtnStyle = canClaim? '' : 'opacity:0.4'; if(claimed) { claimBtnText = 'DONE'; claimBtnDisabled = true; claimBtnStyle = 'background:linear-gradient(90deg,#22c55e,#16a34a); opacity:1'; }
...
function renderProfile() { document.getElementById('content').innerHTML = `<h2>Profile</h2><div class="card"><p><b>Name:</b> ${user.first_name}</p><p><b>ID:</b> ${user.id}</p><p><b>Total Mined:</b> ${getTotalFarmed().toFixed(4)} TON</p><p><b>Daily Streak:</b> ${dailyStr SAVE_KEY = `minerads_save_${user.id}`;
const TASK_KEY = `minerads_tasks_${user.id}`;
const YOUR_WALLET_ADDRESS = "UQD63olQ9L4WryJy8YJ9kEfO4gaen-GkbtvLy5-co2hkI4kv";
const CLAIM_COOLDOWN = 8 * 3600 * 1000;
const MAX_ADS_PER_DAY = 50;
