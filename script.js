...
const SAVE_KEY=`ads_earn_save_${user.id}`;const YOUR_WALLET_ADDRESS="UQD63olQ9L4WryJy8YJ9kEfO4gaen-GkbtvLy5-co2hkI4kv",MAX_ADS_PER_DAY=50,AD_REWARD_TON=.0002,AD_REWARD_MAD=5,REF_BONUS_TON=.001,REF_BONUS_MAD=100;const TONCENTER_API="https://toncenter.com/api/v2";let tonConnectUI=null,connectedWallet=null,depositInterval=null,processedTxs=JSON.parse(localStorage.getItem(`processed_txs_${user.id}`)||'[]');function getUTCTimestamp(){return Date.now()}let balance=0,madBalance=0,adsWatchedToday=0,lastLoginDay=0,loginStreak=0,depositHistory=[],referredBy=null,myReferrals=[];

const DAILY_REWARDS=[
  {day:1,mad:10,ton:0},{day:2,mad:20,ton:0},{day:3,mad:30,ton:0},{day:4,mad:40,ton:0},
  {day:5,mad:50,ton:0},{day:6,mad:70,ton:0},{day:7,mad:100,ton:.01}
];
...
// REMOVED SHOP AND MINERS TABS
const tabs={home:renderHome,tasks:renderTasks,referral:renderReferral,wallet:renderWallet,profile:renderProfile};
...
function loadGame(){try{const t=localStorage.getItem(SAVE_KEY);if(t){const e=JSON.parse(t);balance=e.balance||0;madBalance=e.madBalance||0;adsWatchedToday=e.adsWatchedToday||0;lastLoginDay=e.lastLoginDay||0;loginStreak=e.loginStreak||0;depositHistory=e.depositHistory||[];referredBy=e.referredBy||null;}}catch{}saveGame()}
function saveGame(){localStorage.setItem(SAVE_KEY,JSON.stringify({balance,madBalance,adsWatchedToday,lastLoginDay,loginStreak,depositHistory,referredBy}))}
...
async function watchAdTask(){if(adsWatchedToday>=MAX_ADS_PER_DAY)return showPopup("alert","Limit","50/50 today");await showRewardedAd();adsWatchedToday++;balance+=AD_REWARD_TON;madBalance+=AD_REWARD_MAD;updateBalance();saveGame();showPopup("success","Earned!",`+${AD_REWARD_TON} TON + ${AD_REWARD_MAD} MAD`);renderTasks()}
...
function renderHome(){
  ...
  c.innerHTML=`<div class="card"><h2>Welcome ${user.first_name}</h2><p>Watch ads and earn rewards</p></div>
  ...
  <div class="card"><h3>📊 Your Stats</h3><p>TON Balance: <b>${balance.toFixed(4)}</b></p><p>MAD Balance: <b>${madBalance.toFixed(0)}</b></p><p>Ads Today: <b>${adsWatchedToday}/${MAX_ADS_PER_DAY}</b></p></div>`
}

// REMOVED SHOP AND MINERS FUNCTIONS

function renderTasks(){const adsReady=typeof window.showGiga==="function";document.getElementById("content").innerHTML=`<h2>Earn</h2><div class="card"><h3>📺 Watch Ads</h3><p>Earn <b>${AD_REWARD_TON} TON + ${AD_REWARD_MAD} MAD</b> per ad</p><p>${adsWatchedToday}/${MAX_ADS_PER_DAY}</p><button class="btn" ${!adsReady?"disabled":""} onclick="watchAdTask()">${adsReady?`WATCH AD +${AD_REWARD_TON} TON`:'LOADING ADS...'}</button></div><div class="card"><h3>💡 Tip</h3><p style="font-size:13px;color:var(--muted)">Come back daily for bonus rewards and invite friends to earn more!</p></div>`}
...
function renderProfile(){document.getElementById("content").innerHTML=`<h2>Profile</h2><div class="card"><p>Name: ${user.first_name}</p><p>Streak: Day ${loginStreak}/7</p><p>TON: ${balance.toFixed(4)}</p><p>MAD: ${madBalance.toFixed(0)}</p><p>Ads Today: ${adsWatchedToday}/${MAX_ADS_PER_DAY}</p>${referredBy?`<p>Invited by: ${referredBy}</p>`:''}</div>`}
