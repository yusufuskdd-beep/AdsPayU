function init(){updateUI();checkDaily();renderMiners();updateTotalIncome();loadTonPrice();initTonConnect();setInterval(updateAllMiners,1e3);document.getElementById('userId').innerText=tg.initDataUnsafe?.user?.id||'Guest';document.getElementById('username').innerText=tg.initDataUnsafe?.user?.username||'-'}
...
function showTab(id){
    ...
    // FIX: Auto-render admin requests when opening admin tab
    if(id === 'admin'){
        renderAdminRequests();
    }
}
